...
import { EditblogAxios, AblogAxios, blogTypesAxios } from 'utils/request/requestInfo.js'
  ...
  setup() {
    ...
    const initData = () => {
      ...
      AblogAxios({
        ...
        .then(
          (res) => {
            const handleRes = res.data.data[0]
            // 页面数据初始化
            title.value = handleRes.blog_title
            description.value = handleRes.blog_info
            imgList.value.push({
              name: '博客封面图',
              url: handleRes.cover_image
            })
            tagsGroup.value = handleRes.blog_tag.split('，')
            type.value = handleRes.type_id
            content.value = handleRes.blog_content
            createTime.value = UTCtobeijing(handleRes.create_time)
    ...
    const beforeRemove = (file) => {
      ...
      // return ElMessageBox.confirm('确定移除图片?', '提示', {
      //   confirmButtonText: '是的',
      //   cancelButtonText: 'yamei~~',
      //   type: 'warning',
      //   center: true
      // })
      //   .then(() => {
